...
' Description:	Random Note Generator with arpeggiation.
'             	Press the Start Button to begin. 
' Developed by Mike Marsh, though mostly ripped off Brice's code
...
RSEED		VAR WORD	' Holds most curentest random number
...
RUN:
    ...
    ' play the note and toggle the AUX pin for gate/trigger
    DAC1V=NOTE  
    ...
    ' play a note one octave above the calculated note
    DAC1V=((MIDI + 12) * 32) - OFFSET  
    TOGGLE AUX  '5V Out used as a Gate/Trigger
    GOSUB LOADALLDACS
    PAUSE TEMPO / 4
    LOW AUX
    ...
    ' play a note one octave below the calculated note
    DAC1V=((MIDI - 12) * 32) - OFFSET
    TOGGLE AUX  '5V Out used as a Gate/Trigger
    GOSUB LOADALLDACS
    PAUSE TEMPO / 4
...
' CALCNOTE
'
' Calculates the next note to play.  The note will be a random number 
' between 48 and 74 (in MIDI note numbers).  Because I don't trust my
' arithmetic skills and I haven't had a chance to run the debugger, I 
' added the bounds check code.  Can you say kludge?  :)
'
' AUTHOR: Mike Marsh
' DATE: 05-05-2004
'************************************************************************
CALCNOTE:
	...
	' get a random number and ensure it's not zero
	RSEED = (RANDOM RSEED) + 1
	
	' Use modulo to scale the number to something between 0 and 48 then add 24
	MIDI = (RSEED // 48) + 24
