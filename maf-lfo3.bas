...
RUN:
	FBEGIN = -1.5707963268
	FEND = 1.5707963268
	FINC = FEND / (TOFLOAT SRATE)
	FMAXVAL= TOFLOAT (MAXOUTPUT)
	FTOGGLE = 1.0
	K = 0
	...
	FOR J = 1 TO 2
		...
		FOR I = 1 TO (2 * SRATE)
			FI = FI + FINC
			LFOFUNC(K) = TOINT (FMAXVAL * (((FTOGGLE * FSIN (FBEGIN + FI)) + 1.0) / 2.0)) 
			K = K + 1
		NEXT
		FTOGGLE = -FTOGGLE 'Flip the wave for the second half of the cycle
	NEXT
...
LFOSTART:
	J = K / 4
	FOR I = 0 TO K
		DAC1V = LFOFUNC(I)		' Send function Voltage to OUT-1 (SIN)
		DAC2V = LFOFUNC(J)		' Send function Voltage to OUT-3 (COS)
		DAC3V = (MAXOUTPUT - LFOFUNC(I))' Send function Voltage to OUT-2 (SIN INVERT)
		DAC4V = (MAXOUTPUT - LFOFUNC(J))' Send function Voltage to OUT-4 (COS INVERT)
		GOSUB LOADALLDACS
		IF I = (K / 2) THEN
			TOGGLE AUX
			TOGGLE RUNLED
